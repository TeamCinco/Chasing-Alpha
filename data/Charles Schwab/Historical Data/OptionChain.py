...
# from functools import partial # Not strictly needed in this refactor
...
print("Environment variables after loading for Options Chain script:")
...
'tickers' : 
    ...
    'save_dir': r"C:\Users\cinco\Desktop\Cinco-Quant\00_raw_data\Options\5.22",
    'app_key': os.getenv('APP_KEY'),
    'app_secret': os.getenv('APP_SECRET'),
    
    # Worker configuration from pricehistoryfast.py
    'max_workers': 24,
    'request_delay': 0.005, # Delay before API call in worker
    'retry_attempts': 1,
    'retry_delay': 1,       # Base delay for retries
    'max_pending_tasks': 555,
    
    # Option Chain specific parameters
    'contract_type': 'ALL',     # Options: 'CALL', 'PUT', 'ALL'
    'strike_count': 17,         # Number of strikes above and below at-the-money
    'strategy': 'SINGLE',       # Options: 'SINGLE', 'ANALYTICAL', 'COVERED', 'VERTICAL', etc.
    'include_quotes': True,     # Whether to include underlying quotes (renamed from includeUnderlyingQuote in some API versions)
    ...
    # Additional parameters for ANALYTICAL strategy (if used)
    'volatility': None,
    'interest_rate': None,
    ...
    'progress_file_name': 'options_progress.json'
}
...
class TokenManager:
    ...
    def save_tokens(self):
        # Assumes lock is held by caller (update_tokens or refresh_access_token)
        data = {
    ...
    def refresh_access_token(self, app_key, app_secret):
        with self.lock: # Acquire lock early for reading refresh_token and updating tokens
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Update tokens under the same lock
                    self.access_token = token_data['access_token']
                    # Schwab might not return a new refresh token unless the old one is expiring soon.
                    # If it's not returned, keep the old one.
                    self.refresh_token = token_data.get('refresh_token', current_refresh_token)
                    self.access_token_expiry = int(time.time()) + token_data.get('expires_in', 1740) - 60 # expires_in is typically 1800s
                    self.refresh_token_expiry = int(time.time()) + token_data.get('refresh_token_expires_in', 7 * 24 * 60 * 60) # Typically 7 days
                    self.save_tokens()
                    print("Access token refreshed successfully.")
                    return True
                print(f"Token refresh API call failed: {response.status_code} - {response.text}")
                if response.status_code in [400, 401]: # Bad request or unauthorized (e.g. bad refresh token)
                    print("Refresh token might be invalid or expired. Full re-authentication might be required.")
                    self.refresh_token = None # Invalidate the bad refresh token
                    self.save_tokens()
                return False
            except Exception as e:
    ...
    def tokens_valid(self):
        with self.lock:
            ...
            # Refresh token doesn't have a strict expiry check for "validity" here, just existence.
            # The refresh_access_token method handles actual refresh token expiry.
        ...
        try:
            ...
            with token_manager.lock:
                ...
                if not token_manager.access_token or \
                   (token_manager.access_token_expiry and current_time >= token_manager.access_token_expiry - 300): # Refresh if expiring in 5 mins
                    needs_refresh = True
            ...
            time.sleep(25) # Wait before retrying loop

# Initialize TokenManager and RateLimiter and add to CONFIG
CONFIG['token_manager'] = TokenManager()
CONFIG['rate_limiter'] = RateLimiter(max_requests=115, time_window=25) # Schwab API limits are often around 120/min
...
def get_auth_token_with_retry(max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            # Extract code from the returned_link
            if 'code=' not in returned_link:
                raise ValueError("The returned URL does not contain an authorization code.")
            
            code_param = returned_link.split('code=')[1]
            auth_code = code_param.split('&')[0] # Get the part before any other params like '&session='
...
def get_option_chain(symbol, access_token, local_config):
    base_url = 'https://api.schwabapi.com/marketdata/v1/chains'
    
    headers = {
        'Authorization': f'Bearer {access_token}',
        'Accept': 'application/json',
        'Schwab-Client-CorrelId': str(uuid.uuid4()), # Recommended for troubleshooting
        # 'Schwab-Resource-Version': '1.0' # Can be specified if needed, often optional
    }
    
    params = {
        'symbol': symbol,
        'contractType': local_config['contract_type'],
        'strikeCount': local_config['strike_count'],
        'includeQuotes': str(local_config['include_quotes']).lower(), # API expects 'true' or 'false'
        'strategy': local_config['strategy']
    }
    
    if local_config.get('from_date'): params['fromDate'] = local_config['from_date']
    if local_config.get('to_date'): params['toDate'] = local_config['to_date']
    if local_config.get('volatility') is not None: params['volatility'] = local_config['volatility']
    if local_config.get('interest_rate') is not None: params['interestRate'] = local_config['interest_rate']
    if local_config.get('days_to_expiration') is not None: params['daysToExpiration'] = local_config['days_to_expiration']
    if local_config.get('underlying_price') is not None: params['underlyingPrice'] = local_config['underlying_price']
    if local_config.get('exp_month') != 'ALL': params['expMonth'] = local_config['exp_month']
    if local_config.get('option_type'): params['optionType'] = local_config['option_type']
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'SUCCESS' and (data.get('callExpDateMap') or data.get('putExpDateMap')):
                 return data
            elif data.get('status') == 'SUCCESS' and not (data.get('callExpDateMap') or data.get('putExpDateMap')):
                print(f"[{symbol} GetOptionChain] Success but no option data returned (e.g., symbol has no options).")
                return data # Return the success response even if empty maps
            else: # Handle other statuses or empty responses
                print(f"[{symbol} GetOptionChain] API reported non-SUCCESS or unexpected structure: {data.get('status', 'N/A')} - {response.text[:200]}")
                return None # Or data, depending on how you want to treat partial failures
        elif response.status_code == 429:
            print(f"[{symbol} GetOptionChain] Error 429: Too Many Requests. {response.text[:200]}")
            raise requests.exceptions.ConnectionError("Rate limit hit (429)")
        elif response.status_code in [401, 403]:
            print(f"[{symbol} GetOptionChain] Auth Error {response.status_code}: {response.text[:200]}")
            raise Exception(f"Auth error {response.status_code} for {symbol}") # Custom exception or re-raise
        else:
            print(f"[{symbol} GetOptionChain] Error: {response.status_code} - {response.text[:200]}")
            return None # Non-retryable error for this specific call
            
    except requests.exceptions.Timeout:
        print(f"[{symbol} GetOptionChain] Request timeout.")
        ...
        print(f"[{symbol} GetOptionChain] Request Exception: {str(e)}")
        raise # Re-raise
    except json.JSONDecodeError as e:
        print(f"[{symbol} GetOptionChain] JSON Decode Error: {str(e)} - Response: {response.text[:200]}")
        # This might happen if response is not valid JSON, could be due to API error page
        return None # Or raise, depending on retry strategy for malformed responses

def get_option_chain_with_retry(symbol, access_token_initial, local_config):
    ...
    for attempt in range(max_retries):
        try:
            if not current_access_token:
                print(f"[{symbol} RetryOpt] No access token at attempt {attempt + 1}. Getting fresh token.")
                current_access_token = local_config['token_manager'].get_access_token()
                if not current_access_token: # Should be rare if refresh thread works
                    print(f"[{symbol} RetryOpt] Failed to get fresh token. Re-authenticating...")
                    current_access_token = get_auth_token_with_retry() # Fallback to full re-auth
                    if not current_access_token:
                         raise Exception("Access token unavailable after full re-auth attempt.")
            
            if rate_limiter: rate_limiter.wait_if_needed()
            
            result = get_option_chain(symbol, current_access_token, local_config)
            # get_option_chain might return None for non-exception "no data" cases or specific handled errors.
            # It might also return a response with status SUCCESS but empty maps.
            # If result is None due to an unhandled API error code (like 500), it was printed in get_option_chain.
            # If an exception was raised (timeout, 429, auth), it's caught below.
            if result is not None: # This means either data or a "SUCCESS but no data" response
                 return result

            # If result is None without an exception, it means get_option_chain decided it's not retryable (e.g. bad symbol, or JSON decode)
            print(f"[{symbol} RetryOpt] get_option_chain returned None without exception. Assuming non-retryable issue for this call.")
            return None # Do not retry if get_option_chain itself handles it as non-retryable

        except requests.exceptions.ConnectionError as e: # Specifically for 429
            print(f"[{symbol} RetryOpt] ConnectionError (likely 429 Rate Limit) on attempt {attempt + 1}: {e}")
            delay = local_config.get('retry_delay', 2) * (3 ** attempt) + random.uniform(0.5, 1.5)
            print(f"[{symbol} RetryOpt] Rate limit likely. Waiting {delay:.2f}s before retry {attempt + 2}...")
            time.sleep(delay)
            current_access_token = local_config['token_manager'].get_access_token() # Refresh token state

        except Exception as e: # Catches timeouts, explicit auth errors from get_option_chain, etc.
            print(f"[{symbol} RetryOpt] Exception on attempt {attempt + 1}: {str(e)}")
            if "Auth error" in str(e) or (hasattr(e, 'response') and e.response and e.response.status_code in [401, 403]):
                print(f"[{symbol} RetryOpt] Auth error detected. Attempting to refresh/re-auth token.")
                # Attempt to refresh. If refresh fails or not possible, get_access_token might trigger full auth later.
                refreshed = local_config['token_manager'].refresh_access_token(local_config['app_key'], local_config['app_secret'])
                if refreshed:
                    current_access_token = local_config['token_manager'].get_access_token()
                else: # Refresh failed, might need full re-auth
                    print(f"[{symbol} RetryOpt] Token refresh failed. Next attempt might try full re-auth.")
                    current_access_token = None # Force re-check/re-auth at start of next loop
            
            if attempt < max_retries - 1:
                delay = local_config.get('retry_delay', 2) * (2 ** attempt) + random.uniform(0.1, 0.5)
                print(f"[{symbol} RetryOpt] Retrying in {delay:.2f} seconds...")
                time.sleep(delay)
            else:
                print(f"[{symbol} RetryOpt] Failed after {max_retries} attempts: {str(e)}")
                return None # Critical failure after retries
    return None


def process_option_data(raw_data, symbol):
    if not raw_data or raw_data.get('status') != 'SUCCESS':
        print(f"[{symbol} ProcessOption] No raw data or status not SUCCESS. Raw: {str(raw_data)[:100]}")
        return None
    
    calls_data = []
    puts_data = []
    
    # Underlying quote data if available
    # underlying_info = raw_data.get('underlying', {}) # Not always present, depends on includeQuotes
    # print(f"Underlying for {symbol}: {underlying_info}")


    # Process call options
    call_exp_date_map = raw_data.get('callExpDateMap', {})
    if not isinstance(call_exp_date_map, dict): call_exp_date_map = {}

    for exp_date_str, strikes_map in call_exp_date_map.items():
        # exp_date = exp_date_str.split(':')[0] # Date is before ':'
        for strike_price_str, contract_list in strikes_map.items():
            # strike_price = float(strike_price_str)
            for contract in contract_list:
                # Add common identifiers
                contract['optionType'] = 'CALL'
                contract['parsedExpirationDate'] = exp_date_str # The key from map, e.g., "2024-06-21:7"
                contract['parsedStrikePrice'] = strike_price_str # The key from map
                contract['underlyingSymbol'] = symbol # The original requested symbol
                calls_data.append(contract)
    
    # Process put options
    put_exp_date_map = raw_data.get('putExpDateMap', {})
    if not isinstance(put_exp_date_map, dict): put_exp_date_map = {}

    for exp_date_str, strikes_map in put_exp_date_map.items():
        # exp_date = exp_date_str.split(':')[0]
        for strike_price_str, contract_list in strikes_map.items():
            # strike_price = float(strike_price_str)
            for contract in contract_list:
                contract['optionType'] = 'PUT'
                contract['parsedExpirationDate'] = exp_date_str
                contract['parsedStrikePrice'] = strike_price_str
                contract['underlyingSymbol'] = symbol
    ...
    if calls_df.empty and puts_df.empty:
        print(f"[{symbol} ProcessOption] No calls or puts contracts found after processing.")
        return None
...
def save_option_data(data_dict, symbol, save_dir):
    if not data_dict or (data_dict['calls'].empty and data_dict['puts'].empty):
        print(f"[{symbol} SaveOption] No data to save.")
        return
    ...
    if not data_dict['calls'].empty:
        ...
        try:
            ...
            print(f"[{symbol} WORKER] Calls data saved to {calls_filename}")
        except Exception as e:
            print(f"[{symbol} WORKER] Error saving calls data for {symbol} to {calls_filename}: {e}")
    ...
    if not data_dict['puts'].empty:
        ...
        try:
            ...
            print(f"[{symbol} WORKER] Puts data saved to {puts_filename}")
        except Exception as e:
            print(f"[{symbol} WORKER] Error saving puts data for {symbol} to {puts_filename}: {e}")


def print_option_statistics(data_dict, symbol): # Kept as original
    if not data_dict or (data_dict['calls'].empty and data_dict['puts'].empty) :
        return
    print(f"\n[{symbol} Stats] Option Chain Statistics:")
    price_columns = ['strikePrice', 'bid', 'ask', 'last', 'totalVolume', 'openInterest'] # Adjusted for typical option field names

    if not data_dict['calls'].empty:
        print(f"\n[{symbol} Stats] Calls:")
        print(f"Total call contracts: {len(data_dict['calls'])}")
        available_cols = [col for col in price_columns if col in data_dict['calls'].columns]
        if available_cols:
            print(data_dict['calls'][available_cols].describe())
    
    if not data_dict['puts'].empty:
        print(f"\n[{symbol} Stats] Puts:")
        print(f"Total put contracts: {len(data_dict['puts'])}")
        available_cols = [col for col in price_columns if col in data_dict['puts'].columns]
        if available_cols:
            print(data_dict['puts'][available_cols].describe())
...
class ProgressTracker:
    ...
    def mark_completed(self, ticker):
        with self.lock:
            if ticker not in self.completed_tickers:
                ...
                self.save_progress() # Save on each completion
    ...
    def get_completion_percentage(self, all_tickers_list_count):
        with self.lock:
            ...
            # This counts completed tickers that are part of the current run's list
            # Note: if all_tickers_list changes between runs, this might need adjustment
            # For simplicity, we assume all_tickers_list is fixed for a run.
            completed_count = len(self.completed_tickers) 
            # To be more precise if progress file contains tickers not in current list:
            # relevant_completed_count = len(self.completed_tickers.intersection(set(all_tickers_list)))
            # return (relevant_completed_count / all_tickers_list_count) * 100
            return (completed_count / all_tickers_list_count) * 100


def fetch_and_process_option_ticker(ticker, config, progress_tracker):
    try:
        if progress_tracker and progress_tracker.is_completed(ticker):
            # print(f"[{ticker} WORKER_OPT] Skipping {ticker} - already processed.")
            return None
            
        time.sleep(random.uniform(0.01, config.get('request_delay', 0.05))) # Small random delay
            
        access_token = config['token_manager'].get_access_token()
        if not access_token:
            print(f"[{ticker} WORKER_OPT] CRITICAL: No access token for {ticker}. Triggering re-auth for next attempt if needed.")
            # This might require the main thread or retry logic to handle full re-auth
            return None 
        
        raw_data = get_option_chain_with_retry(ticker, access_token, config)
        
        if raw_data:
            processed_data_dict = process_option_data(raw_data, ticker)
            if processed_data_dict:
                save_option_data(processed_data_dict, ticker, config['save_dir'])
                # print_option_statistics(processed_data_dict, ticker) # Optional: can be verbose
                if progress_tracker:
                    progress_tracker.mark_completed(ticker)
                return processed_data_dict # Return the dict of DataFrames
            else:
                print(f"[{ticker} WORKER_OPT] No data processed into DataFrames for {ticker}.")
                # Mark as completed even if no data, to avoid retrying symbols with no options
                if progress_tracker: progress_tracker.mark_completed(ticker)
                return None
        else:
            print(f"[{ticker} WORKER_OPT] No raw data returned from API for {ticker} after retries.")
            if progress_tracker: progress_tracker.mark_completed(ticker) # Mark as completed to avoid retrying
            return None
            
    except Exception as e:
        print(f"[{ticker} WORKER_OPT] CRITICAL EXCEPTION processing {ticker}: {str(e)}")
        import traceback
        traceback.print_exc()
        # Optionally, do not mark as completed on critical failure to allow retry in a future run
        # if progress_tracker: progress_tracker.mark_failed(ticker) # Needs a new method in ProgressTracker
        return None


def main_parallel_options():
    script_config = CONFIG # Use the global CONFIG specific to this script
    print(f"Using save directory: {script_config['save_dir']}")
    setup_directory(script_config['save_dir'])
    
    try:
        print("[MAIN_OPT] Attempting initial authentication...")
        initial_access_token = get_auth_token_with_retry()
        if not initial_access_token:
            print("[MAIN_OPT] CRITICAL: Failed to obtain initial access token. Exiting.")
            return {}
        print("[MAIN_OPT] Initial authentication successful.")
        
        progress_tracker = ProgressTracker(script_config['save_dir'], filename=script_config['progress_file_name'])
        
        config_tickers_list = list(script_config['tickers']) # Ensure it's a list
        ...
        print(f"[MAIN_OPT] Total tickers in config: {total_tickers_in_config}")
        print(f"[MAIN_OPT] Remaining tickers to process: {len(remaining_tickers)}")
        
        if not remaining_tickers:
            print("[MAIN_OPT] All configured tickers have already been processed. Nothing to do.")
            return {}
        
        max_workers = script_config.get('max_workers', 5)
        max_pending = script_config.get('max_pending_tasks', 300)
        results_summary = {} # To store ticker -> success/failure or data summary
        
        print(f"[MAIN_OPT] Starting parallel processing: {max_workers} workers, {max_pending} max pending tasks.")
        batch_size = 300 # Options calls can be heavier, smaller batch
        ...
        for batch_num_idx, batch_start in enumerate(range(0, len(remaining_tickers), batch_size)):
            ...
            print(f"\n[MAIN_OPT] Processing Batch {current_batch_num}/{total_batches} ({len(batch_tickers)} tickers)")
            
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures_map = {
                    executor.submit(fetch_and_process_option_ticker, ticker, script_config, progress_tracker): ticker 
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures_map)):
                    ...
                    try:
                        result = future.result() # result is dict of DFs or None
                        if result:
                            results_summary[ticker_name] = f"Success, Calls: {len(result['calls'])}, Puts: {len(result['puts'])}"
                        else:
                            results_summary[ticker_name] = "Completed with no data or error in worker."
                    except Exception as e_fut:
                        print(f"[MAIN_OPT] Error from completed task for {ticker_name}: {e_fut}")
                        results_summary[ticker_name] = f"Failed with exception: {e_fut}"
                    
                    # Progress Update
                    if (i + 1) % 5 == 0 or (i + 1) == len(futures_map): # Update every 5 or at end of batch
                        overall_comp = progress_tracker.get_completion_percentage(total_tickers_in_config)
                        print(f"[MAIN_OPT] Batch {current_batch_num} Progress: {i+1}/{len(futures_map)}. Overall: {overall_comp:.1f}% ({len(progress_tracker.completed_tickers)}/{total_tickers_in_config})")
            
            if batch_end < len(remaining_tickers):
                delay_between_batches = 3 # Slightly longer for options
                print(f"\n[MAIN_OPT] Batch {current_batch_num} done. Resting {delay_between_batches}s...")
                time.sleep(delay_between_batches)
        
        final_completion = progress_tracker.get_completion_percentage(total_tickers_in_config)
        print(f"\n[MAIN_OPT] Processing COMPLETE! Overall progress: {final_completion:.1f}% ({len(progress_tracker.completed_tickers)}/{total_tickers_in_config})")
        # print(f"[MAIN_OPT] Results summary: {results_summary}") # Can be very long
        print(f"[MAIN_OPT] Number of tickers attempted/processed in this run: {len(results_summary)}")
        return results_summary
    
    except KeyboardInterrupt:
        print("\n[MAIN_OPT] Operation interrupted by user. Progress likely saved. Exiting.")
        return {"status": "interrupted"}
    except Exception as e:
        print(f"[MAIN_OPT] CRITICAL ERROR in main process: {str(e)}")
        import traceback
        traceback.print_exc()
        return {"status": "critical_error", "error": str(e)}

if __name__ == "__main__":
    print(f"Options Chain Script starting. Will process {len(CONFIG['tickers'])} configured tickers.")
    run_results = main_parallel_options()
    if run_results.get("status") in ["interrupted", "critical_error"]:
        print(f"\n[MAIN_OPT_SCRIPT] Script ended with status: {run_results.get('status')}")
    else:
        print(f"\n[MAIN_OPT_SCRIPT] main_parallel_options finished. Processed {len(run_results)} tickers in this session.")