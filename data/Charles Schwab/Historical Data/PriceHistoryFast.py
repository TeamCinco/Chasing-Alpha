...
CONFIG = {
    ...
    'max_workers': 10,  # Number of parallel workers (threads)
    'request_delay': 0.2,  # Delay between API requests in seconds
    'retry_attempts': 3,  # Number of retry attempts for failed requests
    'retry_delay': 1,  # Initial delay for retries (will use exponential backoff)
...
class RateLimiter:
    ...
    def wait_if_needed(self):
        ...
        with self.lock:
            ...
            # If at max requests, wait until we can make another request
...
class TokenManager:
    ...
    def get_access_token(self):
        """Thread-safe method to get a valid access token"""
        with self.lock:
...
def token_refresh_thread(token_manager, app_key, app_secret):
    while True:
        current_time = int(time.time())
        if token_manager.access_token_expiry and current_time >= token_manager.access_token_expiry - 300:  # Refresh 5 minutes before expiry
            print("Refreshing access token...")
            token_manager.refresh_access_token(app_key, app_secret)
        time.sleep(60)  # Check every minute

# Modify the CONFIG to include token management and rate limiter
CONFIG.update({
    'token_manager': TokenManager(),
    'rate_limiter': RateLimiter(max_requests=30, time_window=60)  # Adjust based on Schwab's limits
})
...
def get_price_history_with_retry(symbol, start_date_str, end_date_str, access_token, max_retries=None, rate_limiter=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            result = get_price_history(symbol, start_date_str, end_date_str, access_token)
            
            if result is not None:
                return result
                
            # If no data but no error, don't retry
            if attempt < max_retries - 1 and result is None:
                delay = CONFIG.get('retry_delay', 1) * (2 ** attempt) + random.uniform(0, 1)
                print(f"No data for {symbol}, retrying in {delay:.2f} seconds...")
                time.sleep(delay)
        except Exception as e:
            # If exception, retry with backoff
            if attempt < max_retries - 1:
                delay = CONFIG.get('retry_delay', 1) * (2 ** attempt) + random.uniform(0, 1)
                print(f"Exception for {symbol}: {str(e)}, retrying in {delay:.2f} seconds...")
                time.sleep(delay)
            else:
                print(f"Failed to retrieve data for {symbol} after {max_retries} attempts: {str(e)}")
...
def print_statistics(df, symbol):
    ...
    if len(df) > 0:
        ...
        print("Empty dataset, no statistics to show.")

def fetch_and_process_ticker(ticker, config, access_token, progress_tracker=None):
    """
    Function to fetch and process data for a single ticker.
    This will be run in parallel by multiple threads.
    """
    try:
        print(f"Processing {ticker}...")
        
        # Check if we've already processed this ticker
        if progress_tracker and progress_tracker.is_completed(ticker):
            print(f"Skipping {ticker} - already processed")
            return None
            
        # Get price history data with retry logic
        ...
        if df is not None:
            ...
            # Print statistics (optional in parallel mode - can be noisy)
...
class ProgressTracker:
    """Class to track progress and enable resuming from previous runs"""
    ...
    def load_progress(self):
        ...
        if os.path.exists(self.filename):
            try:
                ...
                print(f"Loaded progress: {len(self.completed_tickers)} tickers already processed")
            except Exception as e:
        ...
        with self.lock:
            try:
                with open(self.filename, 'w') as f:
                    json.dump({'completed_tickers': list(self.completed_tickers)}, f)
            except Exception as e:
    ...
    def get_remaining_tickers(self, all_tickers):
        """Get list of tickers that haven't been processed yet"""
        return [ticker for ticker in all_tickers if ticker not in self.completed_tickers]
        
    def get_completion_percentage(self, all_tickers):
        """Get percentage of completion"""
        total = len(all_tickers)
        completed = len(self.completed_tickers.intersection(all_tickers))
...
def main_parallel():
    ...
    remaining_tickers = progress_tracker.get_remaining_tickers(CONFIG['tickers'])
    ...
    # Define worker count (adjust based on your system and API limits)
    max_workers = CONFIG.get('max_workers', 10)
    
    # Process tickers in parallel
    results = {}
    processed_count = 0
    total_count = len(remaining_tickers)
    
    print(f"Starting parallel processing with {max_workers} workers...")
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Create future for each ticker
        future_to_ticker = {
            executor.submit(
                ...
                access_token,
                progress_tracker
            ): ticker for ticker in remaining_tickers
        }
        ...
        for future in concurrent.futures.as_completed(future_to_ticker):
            ...
            try:
                ...
                if df is not None:
                    results[ticker] = df
                    print(f"✓ Completed {ticker} ({processed_count}/{total_count}, {processed_count/total_count*100:.1f}%)")
                else:
                    print(f"✗ Failed or no data for {ticker} ({processed_count}/{total_count}, {processed_count/total_count*100:.1f}%)")
            except Exception as e:
                print(f"✗ Error processing {ticker}: {str(e)} ({processed_count}/{total_count}, {processed_count/total_count*100:.1f}%)")
    
    # Print final statistics
    completion_percentage = progress_tracker.get_completion_percentage(CONFIG['tickers'])
    print(f"\nProcessing complete! Overall progress: {completion_percentage:.1f}%")
    print(f"Successfully processed: {len(results)} out of {processed_count} attempted tickers")
