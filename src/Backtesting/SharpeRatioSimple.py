...
def calculate_strategy_returns(df, transaction_cost=0.0021666):
    strategy_returns = []
    in_position = False
    entry_price = None
    ...
    for date, day_data in df.groupby(df.index.date):
        ...
        try:
            # Check for entry if we're not in a position
            if not in_position:
            ...
            if in_position:
                ...
                if len(exit_data) > 0:
                    ...
                    if potential_return > 0:
                        ...
                        in_position = False
                        entry_price = None
                    # Otherwise, hold position (do nothing)
    ...
    return pd.DataFrame(strategy_returns, columns=['date', 'return']).set_index('date')
...
def main():
    ...
    # Calculate returns using complete dataset
    strategy_returns = calculate_strategy_returns(full_df)
    ...
    print(f"Daily Buy&Hold Sharpe Ratio: {daily_buyhold_sharpe:.2f}")
    print(f"\nStrategy Total Return: {(strategy_cum_returns.iloc[-1] - 1) * 100:.2f}%")
    print(f"Daily Buy&Hold Total Return: {(daily_buyhold_cum_returns.iloc[-1] - 1) * 100:.2f}%")
    print(f"Single Buy&Hold Total Return: {single_buyhold_return * 100:.2f}%")
